...
module.exports = (app, passport) => {
    ...
    UserModel.findOne({token: req.user.token}, async (err, user) => {
      ...
      axios.post('https://discordapp.com/api/oauth2/token', new URLSearchParams(postData), { headers: {'Content-Type': 'application/x-www-form-urlencoded'} })
          .then(async function (r) {
            ...
            axios.post(`http://nchack.org:1337/user/${user.discord_id}/roles`, {}, { headers: {
                "Authorization": process.env.DISCORD_AUTH_TOKEN,
  ...
  app.post("/api/discord/disconnect", (req, res) => {
    ...
    UserModel.findOne({ token: req.user.token }, async (err, user) => {
      ...
      axios.post(`http://nchack.org:1337/user/${user.discord_id}/disconnect`, {}, { headers: {
          "Authorization": process.env.DISCORD_AUTH_TOKEN
        } });
  ...
  app.post("/challenge/:challenge/enter", async (req, res) => {
    ...
    ChallengeModel.findOne({ _id: req.params.challenge }, async (err, i) => {
      ...
      if (req.user.discord_id) axios.post(`http://nchack.org:1337/user/${req.user.discord_id}/participant`, {}, { headers: {
          "Authorization": process.env.DISCORD_AUTH_TOKEN
        } });
      ...
      for await (let s of top3) {
        if (s.team) {
          ...
          teamMembers.forEach(m => {
            m.flags.winner = true;
            m.save();
            if (m.discord_id) axios.post(`http://nchack.org:1337/user/${m.discord_id}/roles`, {}, { headers: {
                "Authorization": process.env.DISCORD_AUTH_TOKEN,
          ...
          if (user) {
            user.save();
            if (user.discord_id) axios.post(`http://nchack.org:1337/user/${user.discord_id}/roles`, {}, { headers: {
                "Authorization": process.env.DISCORD_AUTH_TOKEN,
                flags: JSON.stringify(user.flags)
              } });
          }
